    ...
    public static void toFileMode(File directorySave) throws IOException {
        try {
            ...
            if (! OliRenderer.skipVid) {
                ...
                String[] fcmd = new String[]{"avconv", "-r", Integer.toString(DimensionHelper.FRAMES_PER_SECOND), "-y", "-i", "./%08d.png", "./video.ogv"};
                String joinD = "";
                for (String k : fcmd) {
                    joinD += " " + k;
                }
                ...
                int errc = p.waitFor();
                if (errc != 0) {
                    throw new RuntimeException("avconv [0] did not suceed, returned errcode " + errc);
                }
                
                rt = Runtime.getRuntime();
                fcmd = new String[]{"avconv", "-y", "-i", "./video.ogv", "-i", "./audio.wav", "-strict", "experimental", "./result.mp4"};
                joinD = "";
                for (String k : fcmd) {
                    joinD += " " + k;
                }
                log.log(Level.INFO, "Running:{0}", joinD);
                p = rt.exec(fcmd, null, directorySave);
                vse = new VoidStream(p.getErrorStream());
                vso = new VoidStream(p.getInputStream());
                vse.start();
                vso.start(); // we don't want to know about stdout & stderr
                errc = p.waitFor();
                if (errc != 0) {
                    throw new RuntimeException("avconv [1] did not suceed, returned errcode " + errc);
                } else {
                    ...
                    new File(outPathS + "video.ogv").delete();
